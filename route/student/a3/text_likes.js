...
router.post("/text/likes", (req, res) => {
    if (!("paragraph" in req.body) || isNaN(req.body["paragraph"]) || req.body["paragraph"] === "") {
        res.status(400).json({ message: "Paragraph is not valid." });
    } else {
        let sql_likes = "UPDATE csc309.text SET likes = (SELECT likes FROM csc309.text WHERE paragraph = ($1) FOR UPDATE) + 1 WHERE paragraph = ($1) RETURNING likes";
